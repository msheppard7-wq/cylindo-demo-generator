  ...
  if (body.payload) {
    ...
    if (payload.type === 'view_submission' && payload.view.callback_id === 'cylindo_demo_submit') {
      ...
      try {
        ...
        // Find a channel to post to — use the user's DM if no channel context
        // Post to the channel where the command was invoked
        // We'll use conversations.open to DM the user with the result
        const dmResult = await slackAPI('conversations.open', { users: userId });
        const channelId = dmResult.data?.channel?.id;

        if (channelId) {
          await slackPostMessage(channelId, `Demo ready for ${brandName}!`, [
            {
              type: 'header',
            ...
            {
              type: 'context',
              elements: [{ type: 'mrkdwn', text: `Generated by <@${userId}> via Cylindo Demo Generator` }],
            },
          ]);
        }
      } catch (error) {
        ...
        const channelId = dmResult.data?.channel?.id;
        if (channelId) {
          await slackPostMessage(channelId, `Error generating demo`, [
            {
              type: 'section',
